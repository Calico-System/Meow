...
# FreePBX extension for the phone (used for MWI mailbox and originate caller ID)
FREEPBX_EXTENSION       = os.environ.get("FREEPBX_EXTENSION", "")
# Channel to originate from when ringing the phone, e.g. PJSIP/200 or Local/200@from-internal
ASTERISK_ORIGINATE_CHANNEL = os.environ.get("ASTERISK_ORIGINATE_CHANNEL", "")
# FreePBX dialplan context for outbound originate (default matches FreePBX's internal context)
ASTERISK_ORIGINATE_CONTEXT = os.environ.get("ASTERISK_ORIGINATE_CONTEXT", "from-internal")
...
def ami_originate(extension: str, callerid: str = "") -> bool:
    """Originate a call from the phone to an extension via FreePBX AMI."""
    ...
    if not extension:
        ...
        return False
    if not ASTERISK_ORIGINATE_CHANNEL:
        print("ami_originate: ASTERISK_ORIGINATE_CHANNEL not set — aborting")
        return False
    if not callerid:
        callerid = f"Calico <{FREEPBX_EXTENSION}>" if FREEPBX_EXTENSION else "Calico"
    action = (
        f"Action: Originate\r\n"
        f"Channel: {ASTERISK_ORIGINATE_CHANNEL}\r\n"
        f"Exten: {extension}\r\n"
        f"Context: {ASTERISK_ORIGINATE_CONTEXT}\r\n"
        f"Priority: 1\r\n"
        f"CallerID: {callerid}\r\n"
        f"Timeout: 30000\r\n"
        f"Async: yes\r\n"
            ...
            while True:
                ...
                while "\r\n\r\n" in buf:
                    ...
                    if ev == "ContactStatus":
                        ...
                        primary_aor = os.environ.get("ASTERISK_PRIMARY_AOR", FREEPBX_EXTENSION)
                        event_aor = (
                            event.get("AOR")
...
def start_discord_bot():
    ...
    async def slash_status(interaction: discord.Interaction):
        ...
        embed.add_field(name="Phone", value=phone_reg, inline=True)
        ami_status = "✅ Connected" if AMI_CONNECTED else "❌ Disconnected"
        embed.add_field(name="FreePBX AMI", value=ami_status, inline=True)
        await interaction.response.send_message(embed=embed)
    ...
    async def slash_call(interaction: discord.Interaction, extension: str):
        ...
        if not AMI_CONNECTED:
            await interaction.response.send_message(embed=discord.Embed(
                ...
                description="AMI is unavailable — check the FreePBX container.",
                color=discord.Color.red()
            ), ephemeral=True)
            return
    ...
    async def on_message(message):
        ...
        if message.author.id in PRIORITY_USER_IDS:
            ...
            if AMI_CONNECTED and FREEPBX_EXTENSION:
                ami_originate(FREEPBX_EXTENSION, f"Priority DM <{FREEPBX_EXTENSION}>")
        else:
            with _DM_LOCK:
                DM_MESSAGE = dm_entry
...
def send_mwi(waiting: bool):
    ...
    if not MWI_ENABLED:
        return
    if not FREEPBX_EXTENSION:
        print("MWI: FREEPBX_EXTENSION not set — skipping MWI")
        return
    ...
    action = (
        ...
        f"Mailbox: {FREEPBX_EXTENSION}@default\r\n"
