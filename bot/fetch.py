...
# ── SIP number validation ───────────────────────────────────────────────────
_SIP_NUMBER_RE = re.compile(r'^[0-9+*#]+$')

def _validate_sip_number(value: str, context: str) -> str:
    """Validate a SIP extension number for safe interpolation into pjsip.conf.
    ...
    if not value or not _SIP_NUMBER_RE.match(value):
        raise ValueError(
            f"Invalid SIP extension number for {context!r}: {value!r} — "
...
auth=oak-line1
aors={line1_number}
callerid="{ASTERISK_LINE1_DISPLAYNAME}" <{line1_number}>

[oak-line1](auth_template)
password={ASTERISK_LINE1_SECRET}
username=oak-line1

[{line1_number}](aor_template)
{line_blocks}
