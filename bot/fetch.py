...
PHONE_MAC = os.environ.get("PHONE_MAC", "").upper().replace(":", "").replace("-", "")
...
ASTERISK_SCCP_PORT    = int(os.environ.get("ASTERISK_SCCP_PORT", "2000"))
...
# SIPcord trunk (external line, routed through Asterisk)
SIPCORD_USERNAME    = os.environ.get("SIPCORD_USERNAME", "")
SIPCORD_PASSWORD    = os.environ.get("SIPCORD_PASSWORD", "")
SIPCORD_PROXY       = os.environ.get("SIPCORD_PROXY", "bridge-eu1.sipcord.net")
SIPCORD_LINE_NUMBER = os.environ.get("SIPCORD_LINE_NUMBER", "1001")
...
_SIP_NUMBER_RE = re.compile(r'^[0-9+*#]+$')

def _validate_sip_number(value: str, context: str) -> str:
    """Validate an extension number for safe interpolation into Asterisk configs.

    Allowed characters: digits, +, *, #.  Raises ValueError if the value is
    empty or contains anything else, preventing config injection via env vars.
    """
    if not value or not _SIP_NUMBER_RE.match(value):
        raise ValueError(
            f"Invalid extension number for {context!r}: {value!r} — "
...
def _sanitize_xml_attr(s):
    """Escape special characters for use inside a double-quoted XML attribute value."""
...
def _strip_invalid_xml_chars(s):
    """Remove ASCII control characters that are not legal in XML 1.0 documents."""
...
def write_xml_refresh(filename, title, text, refresh_secs, refresh_url):
    xml = f'''{_XML_DECL}<CiscoIPPhoneText Refresh="{refresh_secs}" URL="{_sanitize_xml_attr(refresh_url)}">
...
def write_xml(filename, title, text):
    ...
    xml = f"""{_XML_DECL}<CiscoIPPhoneText Refresh="{IDLE_CYCLE_SECONDS}" URL="{_sanitize_xml_attr(idle_url)}">
  <Title>{_sanitize_xml(title)}</Title>
  <Prompt>Updated: {datetime.now().strftime('%H:%M')}</Prompt>
  <Text>{_to_phone_text(text)}</Text>
</CiscoIPPhoneText>"""
...
def write_asterisk_configs():
    ...
    try:
        ...
        # logger.conf — always written; no secrets; must exist before Asterisk starts
...
"""
        ...
        # stasis.conf — always written; no secrets; must exist before Asterisk starts
...
"""
        ...
        # asterisk.conf — always written; no secrets; must exist before Asterisk starts
...
"""
        ...
        # modules.conf — always written; no secrets; must exist before Asterisk starts
...
"""
    ...
    if not ASTERISK_LINE1_SECRET:
        print("Asterisk: ASTERISK_LINE1_SECRET not set - skipping SCCP/dialplan config generation")
        return
    try:
        if not PHONE_MAC:
            raise ValueError(
                "PHONE_MAC is not set — required to generate the SCCP device ID in skinny.conf. "
                "Set PHONE_MAC to your phone's MAC address (no colons, any case)."
            )
        device_id = f"SEP{PHONE_MAC}"

        line1_number = _validate_sip_number(ASTERISK_LINE1_NUMBER, "ASTERISK_LINE1_NUMBER")

        # Validate SIPcord line number once if the trunk is configured
        sipcord_num = None
        if SIPCORD_USERNAME and SIPCORD_PASSWORD:
            sipcord_num = _validate_sip_number(SIPCORD_LINE_NUMBER, "SIPCORD_LINE_NUMBER")

        # ── skinny.conf ───────────────────────────────────────────────────────
        # Additional SCCP lines from ASTERISK_LINES (line 2–10)
        extra_line_defs = ""
        extra_line_refs = ""
        for name, number, secret, display in ASTERISK_LINES:
            number = _validate_sip_number(number, name)
            extra_line_defs += f"""
[{name}]
label={display}
...
cid_name={display}
"""
            extra_line_refs += f"line => {name}\n"

        # SIPcord outbound line (line 2 on the phone, routed through PJSIP trunk)
        sipcord_line_def = ""
        sipcord_line_ref = ""
        if sipcord_num is not None:
            sipcord_line_def = f"""
; ── SIPcord — external line routed through PJSIP trunk ───────────
[sipcord-line]
label=SIPcord
shortname=L2
context=sipcord-outbound
cid_num={sipcord_num}
cid_name=SIPcord
"""
            sipcord_line_ref = "line => sipcord-line\n"

        skinny = f"""; Auto-generated by fetch.py — do not edit manually

[general]
bindaddr=0.0.0.0
port={ASTERISK_SCCP_PORT}
keepalive=60
debug=off
allow=ulaw
allow=alaw
...
[oak-line1]
label={ASTERISK_LINE1_DISPLAYNAME}
shortname=L1
context=calico
cid_num={line1_number}
cid_name={ASTERISK_LINE1_DISPLAYNAME}
mailbox={line1_number}@default
{extra_line_defs}{sipcord_line_def}
[devices]
...
allow=alaw
line => oak-line1
{extra_line_refs}{sipcord_line_ref}"""
        skinny_conf_path = os.path.join(ASTERISK_CONFIG_DIR, "skinny.conf")
        with open(skinny_conf_path, "w") as f:
            f.write(skinny)
        os.chmod(skinny_conf_path, 0o600)

        # ── pjsip.conf — SIPcord trunk only ──────────────────────────────────
        if sipcord_num is not None:
            pjsip = f"""; Auto-generated by fetch.py — do not edit manually
; SIPcord trunk — used for external calls from the SCCP phone's line 2.
...
[sipcord-registration]
type=registration
transport=sipcord-transport
outbound_auth=sipcord-auth
server_uri=sip:{SIPCORD_PROXY}
client_uri=sip:{sipcord_num}@{SIPCORD_PROXY}
retry_interval=60

[sipcord-auth]
type=auth
auth_type=userpass
username={SIPCORD_USERNAME}
password={SIPCORD_PASSWORD}

[sipcord-aor]
type=aor
contact=sip:{SIPCORD_PROXY}

[sipcord]
type=endpoint
transport=sipcord-transport
context=sipcord-inbound
disallow=all
allow=ulaw
allow=alaw
outbound_auth=sipcord-auth
...
"""
            ...
            with open(pjsip_conf_path, "w") as f:
                f.write(pjsip)
            os.chmod(pjsip_conf_path, 0o600)

        # ── extensions.conf ───────────────────────────────────────────────────
        sipcord_contexts = ""
        if sipcord_num is not None:
            sipcord_contexts = f"""
; Outbound calls from SCCP line 2 to SIPcord trunk
[sipcord-outbound]
exten => _X,1,Dial(PJSIP/${{EXTEN}}@sipcord,30)
exten => _X,2,Hangup()
exten => _X.,1,Dial(PJSIP/${{EXTEN}}@sipcord,30)
exten => _X.,2,Hangup()

; Inbound calls from SIPcord to SCCP phone line 2
[sipcord-inbound]
exten => _X.,1,Dial(Skinny/oak@sipcord-line,30)
exten => _X.,2,Hangup()
exten => s,1,Dial(Skinny/oak@sipcord-line,30)
exten => s,2,Hangup()
"""

        ext_conf = f"""; Auto-generated by fetch.py — do not edit manually

; Internal calls via SCCP (line 1)
[calico]
exten => _X,1,Dial(Skinny/oak@oak-line1,30)
exten => _X,2,Hangup()
exten => _X.,1,Dial(Skinny/oak@oak-line1,30)
exten => _X.,2,Hangup()
{sipcord_contexts}"""
...
"""
        ...
        with open(manager_conf_path, "w") as f:
            f.write(manager)
        os.chmod(manager_conf_path, 0o600)
...
def _fetch_page10_impl():
    ...
    all_results = []
    ex = concurrent.futures.ThreadPoolExecutor(max_workers=min(len(text_channels), 20))
    futures = []
    try:
...
def ami_connect():
    ...
    try:
        ...
        sock.recv(1024)  # banner — still within the 5 s handshake timeout
        login = (
            f"Action: Login\r\n"
        ...
        if "Success" not in resp:
            ...
            sock.close()
            return None
        # Remove the timeout only after the full handshake so the event-loop
        # recv() can block indefinitely waiting for events.
...
def ami_originate(extension: str, callerid: str = "Calico <2001>") -> bool:
    ...
    action = (
        f"Action: Originate\r\n"
        f"Channel: Skinny/oak@oak-line1\r\n"
        f"Exten: {extension}\r\n"
        f"Context: calico\r\n"
        f"Priority: 1\r\n"
        f"CallerID: {callerid}\r\n"
...
def send_mwi(waiting: bool):
    ...
    if waiting == MWI_STATE:
        return
    new_messages = "1" if waiting else "0"
    action = (
        f"Action: MessageWaiting\r\n"
        f"Mailbox: {ASTERISK_LINE1_NUMBER}@default\r\n"
    ...
    if ami_send(action):
        MWI_STATE = waiting
        print(f"MWI {'ON' if waiting else 'OFF'} sent via AMI")
    else:
        print("MWI send error: AMI not connected")
